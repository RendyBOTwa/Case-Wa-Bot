    ...
    lolhuman.on('group-participants-update', async(chat) => {
        try {
            ...
            if (chat.action == 'add') {
                ...
                welkam = `${ini_user.notify}, Welkam gan jan lupa sarapan ${group_info.subject}`
...
if (isGroup && isAntilink) {
	if (budy.includes("://chat.whatsapp.com/")) {
		...
		reply(` *LINK DETECTOR* \n\nKamu nakal si! Ngapain kirim link grup lain? \nMaaf tapi aku harus mengeluarkan kamu...\nSelamat tinggal~`)
				 lolhuman.groupRemove(from, [sender])
}
	}
            ...
            switch (command) {
            	case 'antilink':
					if (!isGroup) return reply('grup only su..')
              ...
              case 'menu':
                case 'help':
                    var punya_wa = "0@s.whatsapp.net"
                    var ini_text = "SANSLINE PROJECTS"
                    var ini_buffer = await getBuffer("https://i.ibb.co/1Z5R2Hc/IMG-20210810-035658-min.jpg")
                    const ini_csreply = {
                        contextInfo: {
               ...
               case 'setwelcome':
					...
					join = body.slice(11)
					lolhuman.sendMessage(from ,`\`\`\`Welcome berhasil di ubah menjadi : ${body.slice(11)}\`\`\``, text,{quoted : freply})
				break 
                case 'tagstick':
                    if (sender.split("@")[0] != owner) return reply("Command only for owner bot")
